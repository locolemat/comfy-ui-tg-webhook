...
class Queue(Base):
    ...
    @classmethod
    def get_server_queue(cls, address, session):
        return session.scalars(select(Queue).where(Queue.server_address == address).where(Queue.processed == 0))
    ...
    def get_users_queue_length(cls, user_id):
        with Session(queue_engine) as session:
            return session.query(Queue).where(Queue.user_id == user_id).count()
