...
async def prompt_query(prompt, address, id, width, height, workflow, model=None, frames=None):

    async with aiohttp.ClientSession() as session:
        response = await session.post(f"http://{address}/prompt", 
                                      json=workflow.get_workflow(prompt=prompt, id=id, negative_prompt="", width=width, height=height, frames=frames, model=model, seed=randint(1, 1_000_000_000)))
